...
generate_cr() {
    ...
    if is_cr_spec "$spec_name"; then
        log "${RED}CR failed - not generating CR-of-CR${NC}"
...
- [ ] npm run build succeeds"
    ...
    echo "$prompt" | timeout 300 $AI_CMD -p > /dev/null 2>&1
...
Be concise - max 10 lines."
    ...
    local result
    result=$(echo "$prompt" | $AI_CMD -p --image "$screenshot" 2>&1) || true
    ...
    log "${YELLOW}Design issues found${NC}"
    echo "$result" | head -10
...
generate_design_cr() {
    ...
    # Prevent CR-of-CR
    if is_cr_spec "$spec_name"; then
        log "${RED}Design CR failed - not generating CR-of-CR${NC}"
        return 1
    fi
